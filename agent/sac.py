...
class SAC(Agent):
    ...
    def __init__(self, obs_space, obs_dim, action_dim, action_range, device, 
        ...
        self.s_ent_stats = pebble.TorchRunningMeanStd(shape=[1], device=device)
    ...
    def get_action(self, obs, sample=False, determ=False):
    ...
    def update_critic(self, obs, action, reward, next_obs, 
        ...
        if self.action_type == 'Cont':
            with torch.no_grad():
                next_action, log_prob, _ = self.actor.get_action(next_obs)
            ...
            with torch.no_grad():
                _, log_prob, action_probs = self.actor.get_action(next_obs)
    ...
    def update_actor_and_alpha(self, obs, logger, step, print_flag=False):   
        if self.action_type == 'Cont':
            ...
            with torch.no_grad():
                _, log_prob, action_probs = self.actor.get_action(obs)
